...
class Encoder_block(nn.Module):
    ...
    def __init__(self, dim_model, heads_en, total_ex, total_cat, total_tg, seq_len):
        ...
        self.ts_fc = nn.Linear(1, dim_model, bias=False)
    ...
    def forward(self, in_ex, in_cat, in_tg, in_dt, in_ts, first_block=True):
        ...
        if first_block:
            ...
            in_ts = in_ts.unsqueeze(-1)
            in_ts = self.ts_fc(in_ts)
            ...
            out = torch.cat([in_ex, in_cat, in_dt, in_ts, (avg_in_tg_embed + max_in_tg_embed)], axis=2)
