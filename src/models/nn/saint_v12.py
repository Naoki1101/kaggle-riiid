...
class Encoder_block(nn.Module):
    ...
    def forward(self, in_ex, in_cat, in_tg, in_dt, in_ts, ts_mask, first_block=True):
        ...
        ts_mask = torch.cat([ts_mask] * self.heads_en, dim=0)
...
class SAINT(nn.Module):
    ...
    def forward(self, feat):
        ...
        ts_mask = feat['ts_mask']

        first_block = True
        for x in range(self.num_en):
            if x >= 1:
                first_block = False
            in_ex = self.encoder[x](in_ex, in_cat, in_tg, in_dt, in_ts, ts_mask, first_block=first_block)
            in_cat = in_ex
