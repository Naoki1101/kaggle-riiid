...
class Encoder_block(nn.Module):
    ...
    def __init__(self, dim_model, heads_en, total_ex, total_cat, seq_len):
        ...
        self.fc_tc = nn.Linear(self.seq_len, dim_model)
    ...
    def forward(self, in_ex, in_cat, in_tc, first_block=True):
        device = in_ex.device
        ...
        in_tc = self.fc_tc(in_tc)
        out = out + in_tc.unsqueeze(1)
...
class SAINT(nn.Module):
    ...
    def forward(self, feat):
        ...
        in_cat = feat['in_cat']
        in_in = feat['in_de']
        in_tc = torch.sigmoid(feat['in_tc'].max(dim=1, keepdims=True).values - feat['in_tc'].float())

        first_block = True
        for x in range(self.num_en):
            if x >= 1:
                first_block = False
            in_ex = self.encoder[x](in_ex, in_cat, in_tc, first_block=first_block)
            in_cat = in_ex
