...
class Encoder_block(nn.Module):
    ...
    def forward(self, in_ex, in_cat, in_tg, in_dt, in_task, first_block=True):
        device = in_ex.device
...
class Decoder_block(nn.Module):
    ...
    def forward(self, in_in, in_el, in_exp, en_out, first_block=True):
        device = in_in.device
...
class SAINT(nn.Module):
    ...
    def forward(self, feat):
        ...
        for x in range(self.num_en):
            if x >= 1:
                first_block = False
            in_ex = self.encoder[x](in_ex, in_cat, in_tg, in_dt, in_task, first_block=first_block)
            in_cat = in_ex
        ...
        for x in range(self.num_de):
            if x >= 1:
                first_block = False
            in_in = self.decoder[x](in_in, in_el, in_exp, en_out=in_ex, first_block=first_block)
